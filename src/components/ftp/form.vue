<template>
  <div>
    <el-dialog title="添加一个FTP代理" :visible.sync="dialogFormVisible" @close='closeForm'>
      <el-form :model="form">
...
export default {
  ...
  methods: {
    ...
    createByForm(){
      ...
      this.$service.createOneFtp(this.form).then(
      (res)=>{
        console.log(res)
        if(res.data.code==0){
          Message.success('添加Ftp成功！本地端口：'+res.data.realLocalPort)
        }else{
