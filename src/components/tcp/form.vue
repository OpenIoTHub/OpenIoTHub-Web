<template>
  <div>
    <el-dialog title="添加一个TCP代理" :visible.sync="dialogFormVisible" @close='closeForm'>
      <el-form :model="form">
...
export default {
  ...
  methods: {
    ...
    createByForm(){
      ...
      this.$service.createOneTcp(this.form).then(
      (res)=>{
        console.log(res)
        if(res.data.code==0){
          Message.success('添加TCP成功！本地端口：'+res.data.realLocalPort)
        }else{
